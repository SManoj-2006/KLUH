...
const mapJob = (job: SupabaseJobRow) => {
...
router.get('/', authenticateToken, async (req: AuthRequest, res) => {
  try {
    ...
    let jobsData = (data || []).map((job) => mapJob(job as SupabaseJobRow));
    ...
    if (skills && typeof skills === 'string') {
      ...
      jobsData = jobsData.filter((job) =>
        skillList.some((skill) => job.skills.some((jobSkill) => jobSkill.toLowerCase().includes(skill)))
      );
    }
    ...
    const jobsWithMatchScore = jobsData.map((job) => {
      ...
      const dynamicMatch =
        job.skills.length > 0 ? Math.round((matchingSkills.length / job.skills.length) * 100) : 0;
...
router.get('/:id', authenticateToken, async (req: AuthRequest, res) => {
  try {
    ...
    const job = mapJob(data as SupabaseJobRow);
    const userSkills = await getUserSkills(req.accessToken!, req.user!.id);
    const matchingSkills = job.skills.filter((skill) =>
      userSkills.some((userSkill) => userSkill.toLowerCase() === skill.toLowerCase())
    );
